...
rhRankedFile = globalPath + '\\DailyStockReport\\' + ds + '-OBV_Ranked.csv'
...
for row in csvData[1:120]:
    ...
    # print ("INSERT INTO stock_obv_value (tickerID, obv_date, obv_value, close_diff, spread, ranked) VALUES (%s, %s, %s, %s, %s, %s)", (tickerID, fileDate.strftime('%Y-%m-%d %H:%M:%S'), OBV_Value, Close_Diff, Spread, Rank))
    # exit()

    cursor = cnx.cursor()
    try:
        cursor.execute("INSERT INTO stock_obv_value (tickerID, obv_date, obv_value, close_diff, spread, ranked) "
                       "VALUES (%s, %s, %s, %s, %s, %s)", (tickerID, fileDate.strftime('%Y-%m-%d %H:%M:%S'), OBV_Value, Close_Diff, Spread, Rank))
...
top20_sql = "select tickerID from stock_obv_value as a where a.obv_date = (select max(b.obv_date) from stock_obv_value as b) " \
            "and a.listed = 1 order by a.obv_date, a.ranked limit 120;"

cursor = cnx.cursor()
cursor.execute(top20_sql)
...
for ticker in tickers:
    ...
    print(url)
    req = Request(url=url, headers={'user-agent': 'my-app/0.0.1a'
                                    })
...
parsed_and_scored_news.to_csv(globalPath + "\\DailyStockReport\\" + ds + "-Sentiment_Ranked.csv", index=False)
