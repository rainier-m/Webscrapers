...
'''
# Commenting out pull of Robinhood Sentiment File because no longer being maintained / updated
...
cursor = cnx.cursor()
cursor.execute("SELECT tickerID FROM monitored WHERE status = 1")
...
while (iter < searchStocks) and (API_Calls < maxAPIcalls):
    ...
    if (os.path.isfile(globalPath + dataPath + stock +".csv") == True) and (fileDateCheck.days > 0):
        print ("File Older than One Day:", stock)
        try:
            # stock = tickers[iter][0]
